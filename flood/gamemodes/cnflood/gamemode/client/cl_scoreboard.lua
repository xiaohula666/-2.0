...
function GM:HUDDrawScoreBoard()
	...
	-- Titles
	draw.SimpleText("名字", ScoreboardFont, xOffset + 16, ySpacing, color_black)
	draw.SimpleText("钱包", ScoreboardFont, xOffset + boardWidth - (colWidth*3) + 8, ySpacing, color_black)
	draw.SimpleText("死亡次数", ScoreboardFont, xOffset + boardWidth - (colWidth*2) + 8, ySpacing, color_black)
	draw.SimpleText("延迟", ScoreboardFont, xOffset + boardWidth - (colWidth*1) + 8, ySpacing, color_black)

	ySpacing = ySpacing + 22

	local yPosition = ySpacing
	for team,info in pairs(ScoreboardInfo) do
		local teamText = info.TeamName .. "  (" .. #info.Players .. "名玩家)"
