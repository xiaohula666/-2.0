...
function GM:HUDPaint()
	...
	if BuildTimer and FloodTimer and FightTimer and ResetTimer then
		if GameState == 0 then
			...
			draw.SimpleText("等待玩家中...", "Flood_HUD", x * 0.01, y * 0.01, color_white, 0, 0)
			draw.SimpleText("造船时间.", "Flood_HUD", x * 0.01, y * 0.044, color_grey, 0, 0)
			draw.SimpleText("快上船!", "Flood_HUD", x * 0.01, y * 0.078, color_grey, 0, 0)
			draw.SimpleText("消灭敌人!", "Flood_HUD", x * 0.01, y * 0.115, color_grey, 0, 0)
			draw.SimpleText("开始新回合.", "Flood_HUD", x * 0.01, y * 0.151, color_grey, 0, 0)
		else
		...
		if GameState == 1 then
			...
			draw.SimpleText("等待玩家中...", "Flood_HUD", x * 0.01, y * 0.01, color_grey, 0, 0)
			draw.SimpleText("造船时间.", "Flood_HUD", x * 0.01, y * 0.044, color_white, 0, 0)
			draw.SimpleText("快上船!", "Flood_HUD", x * 0.01, y * 0.078, color_grey, 0, 0)
			draw.SimpleText("消灭敌人!", "Flood_HUD", x * 0.01, y * 0.115, color_grey, 0, 0)
			draw.SimpleText("开始新回合.", "Flood_HUD", x * 0.01, y * 0.151, color_grey, 0, 0)
		else
		...
		if GameState == 2 then
			...
			draw.SimpleText("等待玩家中...", "Flood_HUD", x * 0.01, y * 0.01, color_grey, 0, 0)
			draw.SimpleText("造船时间.", "Flood_HUD", x * 0.01, y * 0.044, color_grey, 0, 0)
			draw.SimpleText("快上船!", "Flood_HUD", x * 0.01, y * 0.078, color_white, 0, 0)
			draw.SimpleText("消灭敌人!", "Flood_HUD", x * 0.01, y * 0.115, color_grey, 0, 0)
			draw.SimpleText("开始新回合.", "Flood_HUD", x * 0.01, y * 0.151, color_grey, 0, 0)
		else
		...
		if GameState == 3 then
			...
			draw.SimpleText("造船时间.", "Flood_HUD", x * 0.01, y * 0.044, color_grey, 0, 0)
			draw.SimpleText("快上船!", "Flood_HUD", x * 0.01, y * 0.078, color_grey, 0, 0)
			draw.SimpleText("消灭敌人!", "Flood_HUD", x * 0.01, y * 0.115, color_white, 0, 0)
			draw.SimpleText("开始新回合.", "Flood_HUD", x * 0.01, y * 0.151, color_grey, 0, 0)
		else
		...
		if GameState == 4 then
			...
			draw.SimpleText("快上船!", "Flood_HUD", x * 0.01, y * 0.078, color_grey, 0, 0)
			draw.SimpleText("消灭敌人!", "Flood_HUD", x * 0.01, y * 0.115, color_grey, 0, 0)
			draw.SimpleText("开始新回合.", "Flood_HUD", x * 0.01, y * 0.151, color_white, 0, 0)
		else
	...
	if tr.Entity:IsValid() and not tr.Entity:IsPlayer() then
		if tr.Entity:GetNWInt("CurrentPropHealth") == "" or tr.Entity:GetNWInt("CurrentPropHealth") == nil or tr.Entity:GetNWInt("CurrentPropHealth") == NULL then
			draw.SimpleText("正在获取耐久度...", "Flood_HUD_Small", x * 0.5, y * 0.5 - 25, color_white, 1, 1)
		else
			draw.SimpleText("耐久度: " .. tr.Entity:GetNWInt("CurrentPropHealth"), "Flood_HUD_Small", x * 0.5, y * 0.5 - 25, color_white, 1, 1)
		end
	end

	-- Display Player's Health and Name
	if tr.Entity:IsValid() and tr.Entity:IsPlayer() then
		draw.SimpleText("名字: " .. tr.Entity:GetName(), "Flood_HUD_Small", x * 0.5, y * 0.5 - 75, color_white, 1, 1)
		draw.SimpleText("生命值: " .. tr.Entity:Health(), "Flood_HUD_Small", x * 0.5, y * 0.5 - 60, color_white, 1, 1)
	end
	...
	if LocalPlayer():Alive() and IsValid(LocalPlayer()) then
		...
		draw.SimpleText(math.Max(pHealth, 0).." 生命值","Flood_HUD_B", xSize * 0.5 + (Spacer * 2), y - (ySize * 2.5) - (Spacer * 4), color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		if IsValid(LocalPlayer():GetActiveWeapon()) then
			if LocalPlayer():GetAmmoCount(LocalPlayer():GetActiveWeapon():GetPrimaryAmmoType()) > 0 or LocalPlayer():GetActiveWeapon():Clip1() > 0 then
				...
				draw.SimpleText(wBulletCount.." 子弹", "Flood_HUD_B", xSize * 0.5 + (Spacer * 2), y - ySize - (ySize * 0.5) - (Spacer * 3), color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			else
				draw.RoundedBox(0, Spacer * 2, y - (ySize * 2) - (Spacer * 3), xSize, ySize, Color(30, 105, 105, 255))
				draw.SimpleText("不可使用弹药", "Flood_HUD_B", xSize * 0.5 + (Spacer * 2), y - ySize - (ySize * 0.5) - (Spacer * 3), color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		else
			draw.RoundedBox(0, Spacer * 2, y - (ySize * 2) - (Spacer * 3), xSize, ySize, Color(30, 105, 105, 255))
			draw.SimpleText("没子弹了", "Flood_HUD_B", xSize * 0.5 + (Spacer * 2), y - ySize - (ySize * 0.5) - (Spacer * 3), color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
