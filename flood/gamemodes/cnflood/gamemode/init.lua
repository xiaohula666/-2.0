...
for _, file in pairs (file.Find("cnflood/gamemode/server/*.lua", "LUA")) do
	MsgN("-> "..file)
	include("cnflood/gamemode/server/"..file) 
end

MsgN("Loading Shared Files")
for _, file in pairs (file.Find("cnflood/gamemode/shared/*.lua", "LUA")) do
	MsgN("-> "..file)
	AddCSLuaFile("cnflood/gamemode/shared/"..file)
end

MsgN("Loading Clientside Files")
for _, file in pairs (file.Find("cnflood/gamemode/client/*.lua", "LUA")) do
	MsgN("-> "..file)
	AddCSLuaFile("cnflood/gamemode/client/"..file)
end

MsgN("Loading Clientside VGUI Files")
for _, file in pairs (file.Find("cnflood/gamemode/client/vgui/*.lua", "LUA")) do
	MsgN("-> "..file)
	AddCSLuaFile("cnflood/gamemode/client/vgui/"..file)
end

-- Timer ConVars! Yay!
CreateConVar("flood_build_time", 300, FCVAR_NOTIFY, "Time allowed for building (def: 240)")
CreateConVar("flood_flood_time", 30, FCVAR_NOTIFY, "Time between build phase and fight phase (def: 20)")
CreateConVar("flood_fight_time", 400, FCVAR_NOTIFY, "Time allowed for fighting (def: 300)")
...
-- Cash Convars
CreateConVar("flood_participation_cash", 200, FCVAR_NOTIFY, "Amount of cash given to a player every 5 seconds of being alive (def: 50)")
CreateConVar("flood_bonus_cash", 20000, FCVAR_NOTIFY, "Amount of cash given to the winner of a round (def: 300)")

-- Water Hurt System
CreateConVar("flood_wh_enabled", 1, FCVAR_NOTIFY, "Does the water hurt players - 1=true 2=false (def: 1)")
CreateConVar("flood_wh_damage", 10, FCVAR_NOTIFY, "How much damage a player takes per cycle (def: 1)")

-- Prop Limits
CreateConVar("flood_max_player_props", 30, FCVAR_NOTIFY, "How many props a player can spawn (def: 30)")
CreateConVar("flood_max_donator_props", 30, FCVAR_NOTIFY, "How many props a donator can spawn (def: 30)")
CreateConVar("flood_max_admin_props", 30, FCVAR_NOTIFY, "How many props an admin can spawn (def: 40)")
